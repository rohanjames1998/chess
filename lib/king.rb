...
class King
  ...
  # In the threat detection methods (#diagonal_threat?, #horizontal_threat?, and so on) we first generate all the moves from the possible move location and then check if our king can safely move to this location.
  def diagonal_threat?(move, board)
    # Since these movement methods will return enemy pieces (if any) as their last element (they always return arrays) all we will do is check if the last element is a bishop or queen (these method doesn't return ally pieces).
  ...
  def pawn_threat?(move, board, color)
    ...
    col = move[1].ord - 97
    if color == 'white'
      loc_1 = (row + 1) + (col - 1) #up-left
      loc_2 = (row + 1) + (col + 1) #up-right
    else # For black
      loc_1 = (row - 1) + (col - 1) #down-left
      loc_2 = (row - 1) + (col + 1) #down-right
    end
  ...
  def enemy_pawn?(location, board, color)
    if board[location].is_a?(Pawn) && board[location].color != color
      return true
    end
    return false
  end
